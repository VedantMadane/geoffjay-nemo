...
* [Tailwind-style `<div>` primitives and utility-class styling](plans/tailwind-style-div-primitives.md) - a `<div>` (and sibling primitives) whose `class`/`classes` attribute carries Tailwind-style utilities (`flex flex-col gap-3 bg-red`) mapped at render time to GPUI's fluent `Div` builder. Reuses `apply_layout_styles`/`resolve_color`/design tokens; extends SFC `<style>` with class selectors. **Planned / not implemented.**
